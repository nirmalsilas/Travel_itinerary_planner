...
def generate_endpoint(req: TripRequest):
    ...
    destination_slug = re.sub(r"[^a-z0-9]+", "-", req.destination.lower()).strip("-")[:40]
    date_slug = req.start_date or "trip"
    html_name = f"itinerary_{destination_slug}_{date_slug}_{run_id}.html"
